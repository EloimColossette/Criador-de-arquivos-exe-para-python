...
class ConstrutorDeExe(tk.Tk):
    ...
    def criar_componentes(self):
        ...
        # Linha 5: Módulos Extras
        ttk.Label(quadro_principal, text="Módulos Extras:").grid(row=5, column=0, sticky=tk.W, padx=5, pady=5)
        self.modulos_extras = ttk.Entry(quadro_principal, width=50)
        self.modulos_extras.grid(row=5, column=1, padx=5, pady=5)
        ttk.Button(quadro_principal, text="Selecionar", command=self.selecionar_modulos_extras).grid(row=5, column=2, padx=5, pady=5)

        # Linha 6: Arquivos/Pastas Extras
        ttk.Label(quadro_principal, text="Arquivos/Pastas Extras:").grid(row=6, column=0, sticky=tk.W, padx=5, pady=5)
        self.arquivos_extras = ttk.Entry(quadro_principal, width=50)
        self.arquivos_extras.grid(row=6, column=1, padx=5, pady=5)
        ttk.Label(quadro_principal, text="Opcional").grid(row=6, column=2, sticky=tk.W)
    ...
    def selecionar_modulos_extras(self):
        """Abre diálogo para selecionar múltiplos arquivos .py e adiciona os nomes na entrada."""
        caminhos = filedialog.askopenfilenames(
            title="Selecione os módulos extras",
        ...
        if caminhos:
            ...
            # Junta os nomes separados por vírgula e coloca na entrada
            self.modulos_extras.delete(0, tk.END)
            self.modulos_extras.insert(0, ",".join(nomes_modulos))
    ...
    def criar_exe(self, nome_sistema, caminho_icone, arquivo_principal, diretorio_saida):
        ...
        Funciona de forma genérica, aceitando módulos extras e arquivos/pastas extras.
        """
        # Determina tipo de build
        opcao_build = "--onedir" if self.tipo_build.get() == "onedir" else "--onefile"

        # Monta comando base do PyInstaller
        comando = [
            "pyinstaller",
            opcao_build,
            "--windowed",
        ...
        if hasattr(self, "modulos_extras") and self.modulos_extras.get().strip():
            for mod in self.modulos_extras.get().split(","):
                comando.append(f"--hidden-import={mod}")

        if hasattr(self, "arquivos_extras") and self.arquivos_extras.get().strip():
            for item in self.arquivos_extras.get().split(","):
        ...
        comando.append(f"--add-data={tcl_path};tcl")
        comando.append(f"--add-data={tk_path};tk")
        ...
        try:
            ...
            # Threads para leitura contínua de stdout e stderr
            def ler_stdout():
                for linha in processo.stdout:
                    if linha:
                        self.queue.put(("stdout", linha.strip()))
            ...
            # Se o build foi bem sucedido, copie pastas extras (opcional)
            if processo.returncode == 0:
                if hasattr(self, "pastas_copiar") and self.pastas_copiar:
                    for origem in self.pastas_copiar:
                        origem = origem.strip()
